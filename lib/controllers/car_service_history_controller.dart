...
class CarServiceHistoryController extends GetxController {
  ...
  var carServiceHistoryList = <FuelEntryModel>[].obs;

  Future<void> getCarServiceHistory() async {
    ...
    try {
      MyResponseModel response = await EventsApiCaptain.getCarSrvHistory();

      if (response.statusCode == 200 || response.statusCode == 204) {
         if(response.responseData != null) {
           carServiceHistoryList.value = response.responseData;
        } else {
           carServiceHistoryList.clear();
        }
      } else if (response.statusCode == 403) {
        await _handleTokenRefresh(() => getCarServiceHistory());
