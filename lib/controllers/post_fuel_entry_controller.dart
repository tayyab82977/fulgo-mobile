...
class PostFuelEntryController extends GetxController {
  ...
  Future<void> postFuelEntry({required FuelEntryModel fuelEntryModel}) async {
    ...
    try {
      MyResponseModel response = await EventsApiCaptain.postNewFuelEntry(fuelEntryModel: fuelEntryModel);

      if (response.responseData == true) {
        success.value = true;
      } else if (response.statusCode == 403) {
        await _handleTokenRefresh(() => postFuelEntry(fuelEntryModel: fuelEntryModel));
