...
import 'package:xturbox/blocs/bloc/pickup_bloc.dart';
...
import 'package:xturbox/data_providers/models/paymentReport.dart';
import 'package:xturbox/ui/courier/captainDashboard.dart';
import 'package:xturbox/ui/custom%20widgets/captainOrdersCard.dart';
...
class CaptainOrdersListScreen extends StatefulWidget {
  ...
  CaptainOrdersListScreen({this.resourcesData , this.orderList , this.reserved , this.myReservesBloc , this.index});

  @override
  _CaptainOrdersListScreenState createState() => _CaptainOrdersListScreenState();
}

class _CaptainOrdersListScreenState extends State<CaptainOrdersListScreen> with TickerProviderStateMixin {
  ...
  @override
  void initState() {
    allOrdersList = widget.orderList!.where((element) => element.accepted == null).toList();
    acceptedOrdersList = widget.orderList!.where((element) => element.accepted == true ).toList();
    ...
    _tabController = new TabController(vsync: this ,length: 2 , initialIndex: widget.index!);
  ...
  Widget build(BuildContext context) {
    ...
    checkInBloc = BlocProvider.of<CheckInBloc>(context);
    ...
    BlocProvider(
      ...
      child: BlocConsumer<PickUpBloc , PickUpStates>(
        builder: (context , state){
          if(state is PickupInitial){

            return CreatePickUpSceenTWO(
                pickUpBloc: BlocProvider.of<PickUpBloc>(context)
            );
          }
          ...
          else if (state is PickupSuccess){
            return CreatePickUpSceenTWO(
                pickUpBloc: BlocProvider.of<PickUpBloc>(context)
          ...
          else if (state is ClientCreditLoading){
            return Stack(
              children: [
                CreatePickUpSceenTWO(
                    pickUpBloc: BlocProvider.of<PickUpBloc>(context)
          ...
          else if (state is ClientCreditSuccess){
            return CreatePickUpSceenTWO(
                pickUpBloc: BlocProvider.of<PickUpBloc>(context),
                clientCreditSuccess: state
            );
          }
          return CreatePickUpSceenTWO(
              pickUpBloc: BlocProvider.of<PickUpBloc>(context)
        ...
        listener: (context , state){
          if (state is PickupSuccess) {
            ...
            clientGeneralCreditList = state.calculations!.first;
             ...
             ScaffoldMessenger.of(context).showSnackBar(
                 SnackBar(
                   content: Wrap(
                     ...
                     children: (state.errorList ?? []).map((e) => Text(e,style: TextStyle(color: Colors.white),)).toList(),
                   ),
          ...
          else if (state is ClientCreditFailure){
        ...
        drawer: CaptainDrawer(
          ...
          body: Container(
            ...
            child: Column(
              children: [
                CaptainAppBar(
                  drawerKey:_drawerKey, screenName: 'Single Pickup'.tr(),
                ),
                SizedBox(height: 25,),
                Padding(
                  ...
                  child: Column(
                    children: [
                      Row(
                        ...
                        children: [
                          Expanded(
                            child: AutoSizeText('${widget.orderList![0].senderName}',
                              maxLines: 1,
  ...
  Widget CreatePickUpSceenTWO({PickUpBloc? pickUpBloc, ClientCreditSuccess? clientCreditSuccess , bool loading = false   }){
    return  SafeArea(
      child: Form(
        key: _formKey,
        child: WillPopScope(
                          ...
                          children: [
                            Expanded(
                              child: GestureDetector(
                                onTap: (){
                                  pickUpBloc!.add(GetClientBalance(memberID: widget.orderList![0].member ));
                                          ...
                                          Tab(
                                            child: Container(
                                              ...
                                              child: Row(
                                                mainAxisAlignment: MainAxisAlignment.center,
                                                children: [
                                                  Text("All".tr(), style: TextStyle(color:_tabController!.index == 0 ?Colors.white: Colors.black,fontSize: 16),),
                                                  SizedBox(width: 5,),
                                                  Text("(${allOrdersList.length})", style: TextStyle(color:_tabController!.index == 0 ?Colors.white: Colors.black,fontSize: 14),),
                                                ],
                                              ),
                                            ),
                                          ),
                                          Tab(
                                            child: Container(
                                              ...
                                              child: Row(
                                                mainAxisAlignment: MainAxisAlignment.center,
                                                children: [
                                                  Text("Accepted".tr(), style: TextStyle(color:_tabController!.index == 1 ?Colors.white: Colors.black,fontSize: 16),),
                                                  SizedBox(width: 5,),
                                                  Text("(${acceptedOrdersList.length})", style: TextStyle(color:_tabController!.index == 1 ?Colors.white: Colors.black,fontSize: 14),),
                                                ],
                                              ),
                                            ),
                                          ),
                            ...
                            child: TabBarView(
                              ...
                              children: [
                                        ...
                                        Expanded(
                                          child: Padding(
                                            ...
                                            child: TextFormField(
                                                ...
                                                onChanged: (value){
                                                  filterSearchResults(value , all2 , all3 );
                                                },
                                                controller: editingControllerAll,
                                ...
                                CreateAccaptedTab(
                                  ...
                                  clientCreditSuccess: clientCreditSuccess,
                                  loading: loading
                                ),
  ...
  Widget CreateAccaptedTab({PickUpBloc? pickUpBloc , ClientCreditSuccess? clientCreditSuccess  , bool loading = false}){

    if (clientCreditSuccess != null){
      priceCalculated = true ;

        total =clientCreditSuccess.calculations!.first ;
        suspectedPrice =clientCreditSuccess.calculations!.last ;

        if(clientCreditSuccess.calculations!.first == "0"){
            showReceipt = false ;
                ...
                Expanded(
                  child: ButtonTheme(
                    ...
                    child: FlatButton(
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
                      onPressed: (){
                        pickUpBloc!.add(GetClientCredit(
                          id: widget.orderList![0].member,
                          acceptedList: acceptedOrdersList
                        ),);

                      },
                      color: Constants.blueColor,
                      padding: EdgeInsets.all(20),
                      child: Text('Calculate price'.tr(),
                        style: TextStyle(color: Colors.white),
                      ),
                    ),
                  ),
                                ...
                                if(item.val2 != "B"){
                                  setState(() {
                                    ...
                                    if(item.isSelected){
                                      ...
                                      pickUpBloc?.add(GetReceipt(storeId: widget.orderList?.first.pickupStoreId , method: _currentSelectedPaymentMethod?.val2));
                      ...
                      onPressed: (){
                        if (!priceCalculated) {
                          showDialog(
                              ...
                              builder: (context) {
                                return AlertDialog(
                                  title: Text('Note'.tr()),
                                  content:
                                  Text('please calculate the price first'.tr()),
                                  actions: [
                                    TextButton(
                                      child: Text('ok'.tr()),
                                      onPressed: () {
                          ...
                          if (!loading) {
                            ...
                            pickUpBloc!.add(PickUpThisList(
                                memberId: widget.orderList![0].member,
                                pickupList: acceptedOrdersList,
                                amount: total??'0',
                                receipt: receipt,
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                            ...
                            gradient: LinearGradient(
                                ...
                                colors: [Constants.redColor, Constants.redColor])
                        ),
                        child: Center(child: Text('Pickup'.tr(), style: TextStyle(color: Colors.white, fontSize: 18),)),
                      ),
                        ...
                        Row(
                          children: [
                            IconButton(onPressed: (){
                              pickUpBloc?.add(SendConfirmationMsg(
                                msgType: "1",
                                receiverId: widget.orderList?.first.member,
                                receiverPhone: widget.orderList?.first.senderPhone,
                              ));
