...
class ShipmentTracking extends StatefulWidget {
  ...
  ShipmentTracking(
      {this.resourcesData, this.dashboardDataModel, this.trackingId,this.showBottomBar = true});
  @override
...
class _ShipmentTrackingState extends State<ShipmentTracking> {
  ...
  void initState() {
    if (widget.trackingId != null && widget.trackingId != '') {
      ...
      trackingBloc.add(GetTracking(id: widget.trackingId));
    }
    super.initState();
  }
            ...
            children: [
              Expanded(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                 widget.showBottomBar ? const ClientAppBar() : ClientAppBarNoAction(),
                    Expanded(
                      child: Container(
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                                ...
                                children: [
                                  Expanded(
                                    child: TextFormField(
                                      ...
                                      autofocus:
                                          widget.trackingId != null ? false : true,
                                      onFieldSubmitted: (v) {
                                        if (_formKey.currentState!.validate()) {
                                          trackingBloc.add(
                                              GetTracking(id: idController.text));
                                        }
                                      },
                                      ...
                                      decoration: kTextFieldDecoration2.copyWith(
                                          ...
                                          suffixIcon: IconButton(
                                              icon: Icon(Icons.search , color: Constants.blueColor,),
                                              onPressed: () {
                                                if (_formKey.currentState!
                                                    .validate()) {
                                                  FocusScope.of(context).unfocus();

                                                  trackingBloc.add(GetTracking(
                              ...
                              showLiveTracking  ? ElevatedButton(
                                ...
                                onPressed: () {
                                  Navigator.push(context, MaterialPageRoute(
                                      builder: (context)=>MyMap(shipmentId: id,taker: taker,)));
                                },
                                child: Text('Live tracking'.tr()),
                              ) : SizedBox(),
                              Expanded(
                                child: BlocConsumer<TrackingBloc, TrackingStates>(
                                  bloc: trackingBloc,
                                  ...
                                  listener: (context, state) {
                                    ...
                                    if(state is TrackingLoaded){
                                      if(state.trackingList?.processing?.isNotEmpty ?? false){
                                        if(state.trackingList?.processing?.last.type.toString() == "4"){
                                          setState(() {
                                            if(state.trackingList?.processing?.last.taker != null && state.trackingList?.processing?.last.shipment != null){
                                              showLiveTracking = true ;
                                              id = state.trackingList?.processing?.last.shipment ?? "" ;
                                              taker = state.trackingList?.processing?.last.taker ?? "" ;
                                            }
                                          });
                                        }else{
                                          setState(() {
  ...
  Widget CreateTrackingScreen(TrackingLoaded state){
    return ListView.builder(
      itemCount: 1,
      padding: EdgeInsets.zero,
      itemBuilder: (context, index) {
        return Card(
         child:   _DeliveryProcesses(processes: state.trackingList ?? TrackingList()),
              ...
              return Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    else if(index == 2 && (processes.lost?.isNotEmpty ?? false))
                      Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 10,),
                        child: Text(
                          "Cancelled".tr(),
...
class _InnerTimeline extends StatefulWidget {
  const _InnerTimeline({
    required this.trackingDataModel
  });
            ...
            return Row(
              ...
              children: [
                Flexible(child: Padding(
                  ...
                  child: Column(
                    children: [
                      Text( widget.trackingDataModel[index].typeName ?? "",style: TextStyle(fontSize: 16), ),
                      // Directionality(
                ...
                Column(
                  ...
                  children: [
                    Flexible(child: Text(widget.trackingDataModel[index].reason ?? "",style: TextStyle(fontSize: 17), )),
                    Directionality(
                        textDirection: ui.TextDirection.ltr,
                        child: Text(DateFormat("yyyy-MM-dd hh:mm aaa").format(DateTime.parse(widget.trackingDataModel[index].stamp ??'1974-03-20 00:00:00.000') ) , style: TextStyle(color: Colors.grey , fontSize: 13),)),
                  ],
                ),
              ],
            );
...
class TrackingCard extends StatefulWidget {
  ...
  TrackingCard({this.trackingDataModel, this.resourcesData, this.trackingBloc});
...
class _TrackingCardState extends State<TrackingCard> {
  ...
  String? giver;
  idToName() {
    trackingType = IdToName.idToName('trackType', widget.trackingDataModel!.type.toString());
