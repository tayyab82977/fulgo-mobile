...
import 'package:Fulgox/data_providers/models/NationalAddreesModel.dart';
...
class _BankScreenState extends State<BankScreen> {
  ...
  getProfile() async {
    dashboardDataModel =
        await (userRepository.getUserData() as FutureOr<ProfileDataModel>);
  }

  _onBankButtonPressed() {
    startValidation = true;
    if (_formKey.currentState!.validate()) {
      _editProfileController.addBank(
  ...
  _onChangeBankButtonPressed() {
    if (_bankMemberNameController.text ==
            widget.dashboardDataModelNew?.bankHolder &&
        _ibanController.text == widget.dashboardDataModelNew?.iban &&
        _currentSelectedBank!.id == widget.dashboardDataModelNew?.bank) {
      ComFunctions.showToast(
          color: Colors.red, text: "There is no change in the data".tr());
    } else {
      startValidation = true;

      if (_formKey.currentState!.validate()) {
        _editProfileController.editBank(
            bankName: _currentSelectedBank!.id!,
            iban: "SA" + _ibanController.text,
            name: _bankMemberNameController.text);
      }
  ...
  bool checkBankData() {
    if ((widget.dashboardDataModelNew?.bank == "0" ||
            widget.dashboardDataModelNew?.bank == null) &&
        (widget.dashboardDataModelNew?.bankName == "" ||
            widget.dashboardDataModelNew?.bankName == null) &&
        (widget.dashboardDataModelNew?.bankHolder == "" ||
            widget.dashboardDataModelNew?.bankHolder == null) &&
        (widget.dashboardDataModelNew?.iban == "" ||
            widget.dashboardDataModelNew?.iban == null)) {
      return true;
    } else {
      setState(() {
  ...
  getBankData() {
    setState(() {
      ...
      _additionalNoController.text =
          SavedData.profileDataModel.additional_code ?? "";
      _unitNoController.text = SavedData.profileDataModel.unit ?? "";
      _shortAddressController.text =
          SavedData.profileDataModel.short_address ?? "";
    });
  }
  ...
  void initState() {
    try {
      ...
      _editProfileController.errorMessage.value = 'general';
    }
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Container(
                            ...
                            child: Text(error == 'error' && _editProfileController.errorsList.isNotEmpty ? _editProfileController.errorsList.first.toString().tr() : "Something went wrong please try again".tr())),
                                ...
                                children: [
                                  Expanded(
                                    child: DropdownButtonHideUnderline(
                                      child:
                                          DropdownButtonFormField<Cancellation>(
                                        ...
                                        items: widget.resourcesData?.banks?.map(
                                            (Cancellation dropDownStringItem) {
                                    ...
                                    child: TextFormField(
                                      ...
                                      validator: (String? value) {
                                        ...
                                        if (validateIban(value)) {
                                          return 'please add Iban number with 22 digit'
                                              .tr();
                                        }
                        ...
                        child: TextFormField(
                          decoration: kTextFieldDecoration2.copyWith(
                              ...
                              labelStyle: TextStyle(
                                  fontSize: 14, color: Constants.blueColor)),
                          validator: (String? value) {
                            if (value!.isEmpty) {
                    ...
                    child: MaterialButton(
                        ...
                        onPressed: () {
                          ...
                          if (_addressFormKey.currentState!.validate()) {
                            _editProfileController.addNationalAddress(model);
                          }
                        }),
                  ),
                ],
